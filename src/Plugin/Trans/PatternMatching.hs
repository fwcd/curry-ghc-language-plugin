...
import Language.Haskell.Syntax (noExtField, NoGhcTc)

import GHC.Parser.Annotation (noLocA, noAnn, SrcSpanAnn' (..))
import GHC.Plugins
import GHC.Hs.Binds
...
matchExpr (HsLet _ bs e) = do
  (bs', vss) <- compileLet noSrcSpan bs
...
compileDo ty ctxt (L (SrcSpanAnn _ l) (LetStmt _ bs) : xs) = do
  (bs', vss) <- compileLet l bs
...
compileDo _ _ (L (SrcSpanAnn _ l) (ApplicativeStmt _ _ _) : _) = do
...
compileDo _ _ (L (SrcSpanAnn _ l) (ParStmt _ _ _ _) : _) = do
  reportError (mkMsgEnvelope l neverQualify
    "Parallel list comprehensions are not supported by the plugin")
  failIfErrsM
  return ([], False)
compileDo _ _ (L (SrcSpanAnn _ l) (TransStmt _ _ _ _ _ _ _ _ _) : _) = do
  reportError (mkMsgEnvelope l neverQualify
    "Transformative list comprehensions are not supported by the plugin")
  failIfErrsM
  return ([], False)
compileDo _ _ (L (SrcSpanAnn _ l) (RecStmt _ _ _ _ _ _ _) : _) =  do
  reportError (mkMsgEnvelope l neverQualify
...
applySynExpr (SyntaxExprTc se _ _) ex = HsApp noAnn
  (noLocA (HsPar noAnn (noLocA se)))
  (noLocA (HsPar noAnn (noLocA ex)))

-- | Desugars pattern bindings in the given let-bindings.
-- This is done by introducing selector functions for each of them.
compileLet :: SrcSpan -> HsLocalBinds GhcTc -> TcM ([(RecFlag, LHsBinds GhcTc)], [Located Var])
compileLet _ (HsValBinds _ (XValBindsLR (NValBinds bs _))) = do
...
compileLetBind (L (SrcSpanAnn _ l) (PatBind ty p grhss _)) = do
  mtc <- getMonadTycon
  ...
  where
    ...
    mkSelFun ty' p' pe (old, new) = do
      ...
      let bdy = HsApp noAnn (noLocA (HsPar noAnn (noLocA lam))) pe
          grhs = GRHS noAnn [] (noLocA bdy)
          grhssSel = GRHSs noExtField [noLoc grhs]
            (EmptyLocalBinds noExtField)
          ctxt = FunRhs (noLocA (varName old)) Prefix NoSrcStrict
          alt = Match noAnn ctxt [] grhssSel
    ...
    removeOtherPatterns new p0@(L l1 p1) = L l1 $ case p1 of
      ...
      XPat (CoPat b p2 c)   -> XPat (CoPat b
                                 (unLoc (removeOtherPatterns new (noLocA p2))) c)
...
compileLetBind b@(L (SrcSpanAnn _ l) (FunBind _ (L _ fname)
...
mkSeq v e = do
  ...
  return (noLocA (HsApp noAnn (noLocA (HsApp noAnn
...
mkOtherMatch _ _ _ e ((L (SrcSpanAnn _ l) _, _):_) = do
  reportError (mkMsgEnvelope l neverQualify
...
mkAlts v vs ty1 ty2 err eqs@((vp@(L _ (ViewPat _ e p)), _) : _) = do
  ...
  let viewExpr = HsApp noAnn e (noLocA (HsVar noExtField (noLocA v)))
      pty = hsLPatType p
  ...
  let mgtc = MatchGroupTc [Scaled Many pty] ty2
      ...
      mkWild ex = Match noAnn CaseAlt [noLocA (WildPat pty)] (mkGrhss ex)
  case alts' of
    Left remain -> Right . noLocA <$> matchExpr (mkCse [noLocA (mkWild ex)])
      where
        ex = noLocA (HsCase noExtField (noLocA (HsVar noExtField (noLocA v))) mgI)
        mgtcI = MatchGroupTc [Scaled Many ty1] ty2
        mgI = MG mgtcI (noLocA remain) Generated
    Right ex   -> Right . noLocA <$> matchExpr (mkCse [noLocA (mkWild ex)])
mkAlts v vs ty1 ty2 err eqs@((p, alt) : _)
  | L _ (c@ConPat {}) <- p,
    ...
    isNewTyCon (dataConTyCon dc) = do
      ...
      let grhs = GRHS noAnn [] (noLocA (HsVar noExtField (noLocA v')))
          rhs = GRHSs noExtField [noLoc grhs] (EmptyLocalBinds noExtField)
          match = Match noAnn CaseAlt [p'] rhs
          mgtc = MatchGroupTc [patTy] (varType v')
          mg = MG mgtc (noLocA [noLocA match]) Generated
          cse = noLocA (HsCase noExtField (noLocA (HsVar noExtField (noLocA fresh))) mg)
      ...
      return $ Right $ noLocA $ mkSimpleLet NonRecursive lamApp bdy x (varType x)
  | otherwise = do
...
mkAlts _ _  ty1 _ err   [] =
  let grhs = GRHS noAnn [] err
      grhss = GRHSs noExtField [noLoc grhs] (EmptyLocalBinds noExtField)
      alt = Match noAnn CaseAlt [noLocA (WildPat ty1)] grhss
...
mkNaturalOrSimpleAlt :: Type -> Type -> Var -> LHsExpr GhcTc -> LPat GhcTc
                     ...
                     -> Either [LMatch GhcTc (LHsExpr GhcTc)] (LHsExpr GhcTc)
mkNaturalOrSimpleAlt ty1 ty2 v e1 (L l@(SrcSpanAnn _ l') (LitPat _ lit)) alts
  | isStringLit lit =
  ...
  let ve = HsVar noExtField (noLocA v)
      l' = case neg of
             Just (SyntaxExprTc en [argWN] resWN) ->
               mkHsWrap resWN (HsApp noAnn (noLocA en)
                                (noLocA (mkHsWrap argWN l)))
             _ -> l
      if1 = noLocA (mkHsWrap resWQ
              (HsApp noAnn (noLocA (HsApp noAnn
                (noLocA eq)
...
flattenPat p@(L (SrcSpanAnn _ l) (ListPat (ListPatTc _ (Just _)) _ )) = do
  reportError (mkMsgEnvelope l neverQualify
...
flattenPat (L l (SumPat tys p t a)) =
  (\v -> (L l (SumPat tys (noLocA (VarPat noExtField (noLocA v))) t a), [v], [p]))
  <$> mkVar (tys !! t)
...
flattenPat p@(L (SrcSpanAnn _ l) (SplicePat _ _)) = do
  reportError (mkMsgEnvelope l neverQualify
...
flattenConPatDetails (PrefixCon tyargs args) tys _ = do
  ...
  return (PrefixCon tyargs (map (noLocA . VarPat noExtField . noLocA ) vs), vs, args)
...
mkErrorLit = noLocA . HsLit noAnn . HsString NoSourceText . mkFastString
